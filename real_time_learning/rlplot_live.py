...
class MainLiveRL():
    ...
    # this class is not responsible for timing how often the model is invoked or queried

    # board_shim: The EEG board object that is read to evaluate actions <- changed
    ...
    # update_speed_ms: How often the model is invoked to predict the action from the current EEG data
    ...
    def __init__(self, board_shim, action_functions, filename, num_actions=2, num_samples=125, num_baseline_samples=50, update_speed_ms=50, reference_channels=[], one_hot_value=1e3):
        super().__init__()
        #Sets all paramaters
    ...
    def query_model(self):
        # this function collects the current EEG data and queries the model to predict the action
        data = self.board_shim.get_current_board_data(
            self.num_baseline_samples + self.num_samples)  # Gets the latest num_baseline_samples + num_samples from board
        baseline_data = data[:, :self.num_baseline_samples]  # Isolates the baseline data
        state_data = (data[:, self.num_baseline_samples:].transpose() - np.mean(baseline_data,
                                                                                axis=1)).transpose()  # Subtracts mean of each channel's baseline samples from all samples in each channel
        eeg_data = torch.tensor(state_data[[channel for channel_idx, channel in enumerate(self.eeg_channels) if
                                            channel_idx not in self.reference_channels]])  # Gets the data from all eeg channels except the reference channels
        ...
        if (
                self.last_action != selected_action):  # we only invoke functions when the action changes. this emulates only one click per eeg signal
            try:
